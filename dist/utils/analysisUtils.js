 ...
 */
export function extractTimes(pgn) {
    if (!pgn)
        return [];
    const times = [];
 ...
 */
export function buildPositions(history, startFen = null) {
    const positions = [];
 ...
 */
export function buildAnalysisOrder(total, currentIndex) {
    const order = [];
    const seen = new Set();
    const add = (i) => {
 ...
 */
export function mapLines(lines, isBlackTurn) {
