...
export class GameAnalysisCoordinator {
    ...
    async run(history, currentIndex, gameId, engineConfig = {}, callbacks, extraInfo = {}, prebuiltEngines = null) {
        const { onStatus, onProgress, onMoveResult, onOpeningDetected, onComplete, signal, startFen, } = callbacks;
        ...
        console.log(`[Game] Starting analysis: id=${gameId} | ${history.length} moves | ` +
        ...
        try {
            ...
            const evalResultsRef = [];
            // Clasificar una jugada en cuanto tenga apertura + evaluación
            const tryClassify = (ply) => {
                this._tryClassify(ply, history, positions, evalResultsRef, bookStatus, openingState, finalMoveData, completedSet, onMoveResult, labelCounts, times);
            };
            ...
            const openingPromise = OpeningService.detectOpenings({
                ...
                onOpeningDetected: (data) => {
                    if (data?.openingName)
                        detectedOpening = data.openingName;
                    if (data?.ecoCode)
                        detectedEco = data.ecoCode;
                    onOpeningDetected?.(data);
                },
            });
            openingPromise
