...
export class PuzzleExtractor {
            ...
            if (fullData && Array.isArray(fullData.evaluations) && fullData.evaluations.length > 0) {
                ...
                if (prevDepth >= 16) {
                    ...
                    lightEvalResults = fullData.evaluations.map((e) => {
                        if (!e)
                            return { wp: 0.5, bestMove: '', mate: null };
                        return {
                            wp: e.wp ?? 0.5,
                            bestMove: e.bestMove ?? '',
                            mate: e.mate ?? null
    ...
    _identifyCandidates(history, positions, lightEvalResults) {
        ...
        for (let ply = 0; ply < history.length; ply++) {
            ...
            if (!PUZZLE_LABELS.has(label))
                continue;
            const wpLoss = isWhiteMove ? (before.wp - after.wp) : (after.wp - before.wp);
            if (wpLoss < MIN_WP_LOSS_FOR_PUZZLE)
                continue;
            if (!allowsMate(after.mate) && !isTacticalMove(positions[ply + 1], after.bestMove))
                continue;
            candidates.push({
                ply,
                beforeEval: before,
        ...
        const workers = engines.map(async (engine) => {
            while (true) {
                ...
                try {
                    ...
                    const onlyMove = DataMiner.detectOnlyMove(afterHeavy.lines[0] ?? { score: raw.score, mate: raw.mate }, afterHeavy.lines[1] ?? null, !c.isWhiteMove);
                    if (onlyMove.discard)
                        continue; // Descartar puzzles inválidos de jugada única desastrosa
