...
// ─── Constants & Colors ───────────────────────────────────────────────────────
export const PHASE_ORDER = { 'Apertura': 1, 'Medio Juego': 2, 'Final': 3 };
...
export const StatsRepo = {
    ...
    getAggregatedStats(filters = {}) {
        ...
        for (const row of metricsRows) {
            try {
                ...
                if (am.advantageStatus === 'CONVERTED')
                    acc.convertedAdvantages++;
                if (am.advantageStatus === 'BLOWN_ADVANTAGE')
                    acc.blownAdvantages++;
                if (am.comebackStatus === 'COMEBACK_WIN')
                    acc.comebackWins++;
                if (am.comebackStatus === 'SAVED_DRAW')
                    acc.savedDraws++;
                if (am.comebackStatus === 'FAILED')
                    acc.failedComebacks++;
                if (am.tiltEvents)
                    acc.tiltEvents += am.tiltEvents;
                if (am.timeManagement) {
                    ...
                    if (am.timeManagement.avgMidgameTime > 0)
                        acc.gamesWithTime++;
                }
            }
    ...
    getMoveExplorer(fen) {
        ...
        const formatSide = (sideData) => Object.values(sideData)
            .map(s => ({
            san: s.san,
            count: s.count,
