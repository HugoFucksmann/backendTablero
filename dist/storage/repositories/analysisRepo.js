...
export const AnalysisRepo = {
     ...
     */
    save(entry) {
        const transaction = db.transaction((e) => {
            stmts.insertAnalysis.run(e.id, e.gameId, e.username ?? null, e.createdAt, e.date, e.opening, e.eco ?? null, e.moveCount, e.color, e.win, // 1: Win | 0: Draw | -1: Loss
            e.timeControl, e.white?.accuracy ?? null, e.black?.accuracy ?? null, JSON.stringify(e.accuracyByPhase || []), JSON.stringify(e.labelCounts || {}), JSON.stringify(e.advancedMetrics || {}), e.opponent ?? null, e.gameDate ?? null);
            ...
            if (Array.isArray(e.moves)) {
                for (const move of e.moves) {
                    stmts.insertMove.run(e.id, move.ply, move.san, move.evaluation, move.label, move.moveTime ?? null, move.remaining_time ?? null, move.fen, move.start_fen, move.error_time_class ?? null);
                }
            }
            if (e.fullData) {
