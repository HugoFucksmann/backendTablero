...
const DEFAULT_CONFIG = {
    ...
    multiPv: 1,
    depth: 18,
    maxMovetime: 30000 // [FIX] Evita finalizaciones prematuras hardcodeadas
};
...
class StockfishProcess {
    ...
    async init(config = {}) {
        ...
        if (this._engine.state === EngineState.IDLE) {
            ...
            if (JSON.stringify(merged) !== JSON.stringify(prev)) {
                if (merged.threads !== prev.threads) this._engine.send(`setoption name Threads value ${merged.threads}`);
                if (merged.hash !== prev.hash) this._engine.send(`setoption name Hash value ${merged.hash}`);
                if (merged.multiPv !== prev.multiPv) this._engine.send(`setoption name MultiPV value ${merged.multiPv}`);
                this._config = merged;
            }
    ...
    newGame() {
        if (this._engine.state === EngineState.IDLE) this._engine.send('ucinewgame');
    }
    ...
    async analyzePosition(fen, depth, signal = null, onProgress = null, multiPv = null) {
        ...
        if (!/^[rnbqkpRNBQKP1-8\/\s\-wbea-h0-9]+$/.test(fen)) throw new Error('Malformed FEN string');
        ...
        // [FIX] Mutex Anti-Deadlock
        const previousIdle = this._idlePromise;
        let nextIdleResolve;
        this._idlePromise = new Promise(r => { nextIdleResolve = r; });

        try { await previousIdle; } catch (e) { /* Ignorar fallos de tareas previas */ }

        this._idleResolve = nextIdleResolve;
            ...
            const searchHandler = (line) => {
                ...
                if (bm !== null) {
                    ...
                    if (!settled) {
                        ...
                        settle(null, {
                            score: bestLine.score, mate: bestLine.mate ?? null,
                            bestMove: bm.bestMove || bestLine.move, pv: bestLine.pv,
                ...
                if (info !== null) {
                    ...
                    Object.assign(lines[idx], { depth: d, score, mate, pv, move });

                    if (onProgress) {
                        onProgress({
                            depth: lines[1]?.depth ?? 0, score: lines[1]?.score ?? 0,
                            mate: lines[1]?.mate ?? null, bestMove: lines[1]?.move ?? '',
                            lines: Object.values(lines).sort((a, b) => a.multipv - b.multipv),
                        });
                    }
                }
            ...
            this._engine.lineHandler = (line) => {
                if (line === 'readyok') {
                    if (signal?.aborted || this._engine.state === EngineState.STOPPING) {
                        this._engine.state = EngineState.IDLE;
                        restoreOnDied();
                        return settle(new DOMException('Aborted', 'AbortError'), null);
                    }
                    this._engine.send(`position fen ${fen}`);
                    this._engine.send(`go depth ${depth} movetime ${this._config.maxMovetime}`);
    ...
    async _spawnAndHandshake() {
        ...
        return new Promise((resolve, reject) => {
            const prevOnDied = this._engine.onDied;
            this._engine.onDied = () => { this._onEngineDied(); reject(new Error('Engine died during UCI handshake')); };
